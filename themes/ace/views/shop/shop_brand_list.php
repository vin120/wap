<?php
    $this->pageTitle = Yii::t('vcos','店铺品牌列表');
    $theme_url = Yii::app()->theme->baseUrl;
    $menu_type = 'shop_brand_list';
?>
<?php 
    //navbar 挂件
    $this->widget('navbarWidget');
    if(in_array('343', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('344', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('345', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                            ...
                                            <div class="table-responsive">
                                            	...
                                            	<div class="list_select_option">
                                             	  ...
                                             	  foreach($shop_sel as $row){
                                             	  	$path_url = Yii::app()->createUrl('Shop/shop_brand_list',array('shop'=> $row['shop_id']));
                                             	  ?>
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th><?php echo yii::t('vcos', '店铺')?></th>
                                                            <th><?php echo yii::t('vcos', '品牌')?></th>
                                                            <th><?php echo yii::t('vcos', '排序')?></th>
                                                            <th><?php echo yii::t('vcos', '操作')?></th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <?php foreach ($shop_brand as $key=>$row) { ?>
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['shop_brand_id'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                            location.href="<?php echo Yii::app()->createUrl("Shop/shop_brand_list");?>"+"?id="+$a;
                            $( this ).dialog( "close" );
                        }
                    }
                    ,
