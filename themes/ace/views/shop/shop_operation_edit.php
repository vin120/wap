<?php
    $this->pageTitle = Yii::t('vcos','编辑店铺资质');
    $theme_url = Yii::app()->theme->baseUrl;
    
    $menu_type = 'shop_operation_edit';
?>            
<?php 
    //navbar 挂件
    $disable = 1;
                    ...
                    <div class="row">
                        <div class="col-xs-12 col-sm-12 col-md-11">
                            ...
                            <style>
                            	...
                            	.category_div > ul{overflow-y:auto;list-style:none;width:25%;border:1px solid #ccc;float:left;margin-right:10px;height:450px;}
                            	...
                            	<input type='hidden' name='shop' value="<?php echo $shop['shop_id']?>" />
                             <label>店铺：<?php echo $shop['shop_title']?></label>
                             <?php $already=array(); foreach($already_operation as $row){
                             	$already[] = $row['category_code'];
                             }?>
                             <div class="category_div">
                             	<ul class="cat1_ul">
                             	<?php foreach($cat1_sel as $row){?>
                             	<li class="<?php if($row['category_code']==$cat1_but){echo 'color';}?>"><input class="input_op" type="checkbox" <?php if(in_array($row['category_code'], $already)){echo "checked='checked'";}?> name="code[]" value="<?php echo $row['category_code']?>" /><span class="text"><?php echo $row['name']?><span class="op">〉</span></span></li>
                             	<?php }?>
                             	</ul>
                             	
                             	<ul class="cat2_ul">
                             	<?php foreach($cat2_sel as $row){?>
                             	<li parent="<?php echo $row['parent_cid']?>" class="<?php if($row['category_code']==$cat2_but){echo 'color ';} if($row['parent_cid']!=$cat1_sel[0]['category_code']){echo "hidden";}?>"><input class="input_op" <?php if(in_array($row['category_code'], $already)){echo "checked='checked'";}?> type="checkbox" name="code[]" value="<?php echo $row['category_code']?>" /><span class="text"><?php echo $row['name']?><span class="op">〉</span></span></li>
                             	<?php }?>
                             	</ul>
                             	
                             	<ul class="cat3_ul">
                             	<?php foreach($cat3_sel as $row){?>
                             	<li parent="<?php echo $row['parent_cid']?>" class="<?php if($row['parent_cid']!=$cat2_sel[0]['category_code']){echo "hidden";}?>"><input class="input_op" <?php if(in_array($row['category_code'], $already)){echo "checked='checked'";}?> type="checkbox" name="code[]" value="<?php echo $row['category_code']?>" /><span class="text"><?php echo $row['name']?></span></li>
                             	<?php }?>
                             	</ul>
                             </div>
...
jQuery(function($){
	...
	$('form').submit(function(){
		var flag = 0;
        $("input[class='input_op']").each(function(e){
            if($(this).is(":checked")){
                flag=1;return false;
            }
        });
        if(flag==0){
            alert("没有选中任意一个资质!");return false;
        }
