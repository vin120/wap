<?php
    $this->pageTitle = Yii::t('vcos','游客须知分类列表');
...
<?php 
    ...
    if(in_array('259', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('261', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('263', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                        ...
                                        <div class="col-xs-12">
                                            <div class="table-responsive">
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($notice_to_visitors_category as $key=>$row) { ?>
                                                        <tr>
                                                            <td><?php echo ++$key;?></td>
                                                            ...
                                                            <td>
                                                                <?php
                                                                    ...
                                                                    $this->widget('ManipulateWidget',array(
                                                                        'ControllerName'=>'Safe',
                                                                        'MethodName'=>'notice_to_visitors_category_edit',
                                                                        'id'=>$row['category_id'],
                                                                        'canedit'=>$canedit,
                                                                        'candelete'=>$candelete,
                                                                    ));
                                                ...
                                                <div class="center">
                                                    <?php
                                                        ...
                                                        $this->widget('BotWidget',array(
                                                            'ControllerName'=>'Safe',
                                                            'MethodName'=>'notice_to_visitors_category_add',
                                                            'canadd'=>$canadd,
                                                        ));
                                                    ?>
                                                </div>
            ...
            buttons: [
                {
                    ...
                    "id" : "danger1",
                    click: function() {
                        location.href="<?php echo Yii::app()->createUrl("Safe/notice_to_visitors_category");?>"+"?id="+$a;
                        $( this ).dialog( "close" );
                    }
                }
                ,
