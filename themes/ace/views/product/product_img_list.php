<?php
    $this->pageTitle = Yii::t('vcos','商品图片列表');
    $theme_url = Yii::app()->theme->baseUrl;
    $menu_type = 'product_img_list';
?>
<?php 
    //navbar 挂件
    $this->widget('navbarWidget');
    if(in_array('313', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('314', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('315', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                    ...
                                    <div class="row">
                                        <div class="col-xs-12">
                                            ...
                                            <style>
		                                        ...
		                                        <form method="get" action="<?php echo Yii::app()->createUrl('Product/product_img_list')?>">
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($product_img as $key=>$row) { ?>
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                            location.href="<?php echo Yii::app()->createUrl("Product/product_img_list");?>"+"?id="+$a;
                            $( this ).dialog( "close" );
                        }
                    }
                    ,
