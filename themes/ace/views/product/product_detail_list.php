<?php
    $this->pageTitle = Yii::t('vcos','商品详情列表');
    $theme_url = Yii::app()->theme->baseUrl;
    $menu_type = 'Product_detail_list';
?>
<?php 
    //navbar 挂件
    $this->widget('navbarWidget');
    if(in_array('309', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('310', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('311', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                ...
                <div class="main-content"> 
                    ...
                    <div class="page-content">
                        <div class="page-header">
                            <h1>
                                <?php echo yii::t('vcos', '商品详情管理')?>
                                <small>
                                    <i class="icon-double-angle-right"></i>
                                    <?php echo yii::t('vcos', '商品详情列表')?>
                                </small>
                            </h1>
                        </div><!-- /.page-header -->
                            <div class="row">
                                    ...
                                    <div class="row">
                                        <div class="col-xs-12">
                                            ...
                                            <style>
		                                        ...
		                                        <form method="get" action="<?php echo Yii::app()->createUrl('Product/product_detail_list')?>">
                                            	<div class="list_select_option">
                                             	  ...
                                             	  <select class='city_sel' name='cat3_all_sel'>
                                             	  <?php
                                             	  ...
                                             	  <input style="width:40px;height:28px;border:1px solid #ccc;" type='submit' value='确认' name='submit'/>
                                             	</div>
                                             	</form>
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th><?php echo yii::t('vcos', '商品')?></th>
                                                            <th><?php echo yii::t('vcos', '商品文本详情')?></th>
                                                            <th><?php echo yii::t('vcos', '操作')?></th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <?php foreach ($product_detail as $key=>$row) { ?>
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['product_detail_id'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                                                            <td><?php echo $row['product_name'];?></td>
                                                            <td><?php echo Helper::truncate_utf8_string($row['text_detail'], 20);?></td>
                                                            <td>
                                                                <?php
                                                                    $this->widget('ManipulateWidget',array(
                                                                        'ControllerName'=>'Product',
                                                                        'MethodName'=>'product_detail_edit',
...
<script type="text/javascript">
    jQuery(function($) {
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                            location.href="<?php echo Yii::app()->createUrl("Product/product_detail_list");?>"+"?id="+$a;
                            $( this ).dialog( "close" );
                        }
                    }
                    ,
        ...
        /**改变分类二级,获取三级**/
        $("select[name='cat2_all_sel']").change(function(){
            var this_code = $(this).val();
            var str = '';
            <?php $path_url = Yii::app()->createUrl('Product/GetCategoryChild');?>
            $.ajax({
            	...
            	success:function(data){
            		...
            		$("select[name='cat3_all_sel']").html(str);
