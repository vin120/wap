<?php
    $this->pageTitle = Yii::t('vcos','意见类型列表');
...
<?php 
    ...
    if(in_array('140', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('141', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('142', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                    ...
                                    <div class="col-xs-12">
                                        <div class="table-responsive">
                                            ...
                                            <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                ...
                                                <tbody>
                                                    <?php foreach ($ct as $key=>$row) { ?>
                                                    <tr>
                                                        <td class="center">
                                                            <label>
                                                                <input type="checkbox" name="ids[]" value="<?php echo $row['comment_type_id'];?>" class="ace isclick" />
                                                                <span class="lbl"></span>
                                                            </label>
                                                        </td>
                                                        <td><?php echo ++$key;?></td>
                                                        <td><?php echo $row['comment_type_name'];?></td>
                                                        <td><?php echo $row['comment_type_state']? yii::t('vcos', '启用'):yii::t('vcos', '禁用');?></td>
                                                        <td>
                                                            <?php
                                                                //操作挂件
                                                                $this->widget('ManipulateWidget',array(
                                                                    'ControllerName'=>'commentandhelp',
                                                                    'MethodName'=>'commenttype_edit',
                                                                    'id'=>$row['comment_type_id'],
                                                                    'canedit'=>$canedit,
                                                                    'candelete'=>$candelete,
                                                                ));
                                                            ?>
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                            location.href="<?php echo Yii::app()->createUrl("Commentandhelp/commenttype_list");?>"+"?id="+$a;
                            $( this ).dialog( "close" );
                        }
                    }
                    ,
