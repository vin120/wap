<?php
    $this->pageTitle = Yii::t('vcos','编辑美食分类');
...
jQuery(function($){
    ...
    $('form').submit(function(){
        ...
        $id = <?php echo $food_category['food_category_id']?>;
        if($sort!='' && $name!=''){
	        var msg = checkCategoryName($id,$restaurant,$name,$name_iso);
	        if(msg == 0){
			    alert('分类名已存在!');
		       return false;
		    }
...
function checkCategoryName($id,$restaurant,$name,$name_iso=''){
    flag = 1;
    
	$url = "<?php echo Yii::app()->createUrl("Restaurant/CheckCategoryName")?>";
	$.ajax({
		type:'get',
		data:'id='+$id+'&restaurant='+$restaurant+'&name='+$name+'&name_iso='+$name_iso,
		url:$url,
		async: false,
