<?php
    $this->pageTitle = Yii::t('vcos','美食分类管理');
...
<?php 
    ...
    if(in_array('149', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('151', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('153', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                        ...
                                        .restaurant_sel{margin-left:20px;width:250px;}
                                            ...
                                            <div class="list_select_option">
                                             	  ...
                                             	  <select class='restaurant_sel' name='restaurant_all_sel'>
                                             	  	<option value='0' <?php if($res_but == 0){echo 'selected';}?>><?php echo yii::t('vcos', '全部')?></option>
                                             	  <?php foreach($restaurant_sel as $row){
                                             	  	$path_url = Yii::app()->createUrl('Restaurant/category_list',array('restaurant'=> $row['restaurant_id']));
                                             	  ?>
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <?php foreach ($restaurant as $key=>$row) { ?>
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['food_category_id'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                                                            ...
                                                            <td><?php echo $row['food_category_state']?yii::t('vcos', '启用'): yii::t('vcos', '禁用');?></td>
            ...
            buttons: [
                {
                    ...
                    click: function() {
                        location.href="<?php echo Yii::app()->createUrl("Restaurant/category_list");?>"+"?id="+$a;
                        $( this ).dialog( "close" );
                    }
                }
                ,
