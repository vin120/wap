<?php
    $this->pageTitle = Yii::t('vcos','客房分类列表');
...
<?php 
    ...
    if(in_array('30', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('31', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('32', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th><?php echo yii::t('vcos', '客房分类图片')?></th>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td><?php echo $row['room_state']?yii::t('vcos', '启用'):yii::t('vcos', '禁用');?></td>
                                                            <td>
                                                                <?php
                                                                    //操作挂件
                                                                    $this->widget('ManipulateWidget',array(
                                                                        'ControllerName'=>'Room',
                                                                        'MethodName'=>'room_edit',
                                                                        'id'=>$row['room_id'],
                                                                        'canedit'=>$canedit,
                                                                        'candelete'=>$candelete,
                                                                    ));
...
jQuery(function($) {
    ...
    $( ".delete" ).on('click', function(e) {
        var $a = $(this).attr("id");
        e.preventDefault();
        $.post("checkdetail",{id:$a},function(data){
            if(data > 0){
                $('#danger').hide();
            ...
            buttons: [
                {
                    ...
                    "id" : "danger",
                    click: function() {
                        location.href="<?php echo Yii::app()->createUrl("Room/room_list");?>"+"?id="+$a;
                        $( this ).dialog( "close" );
                    }
                }
                ,
