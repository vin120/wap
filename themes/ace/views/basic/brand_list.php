<?php
    $this->pageTitle = Yii::t('vcos','品牌列表');
...
<?php 
    ...
    if(in_array('334', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('335', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('336', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th><?php echo yii::t('vcos', '品牌名')?></th>
                                                            <th><?php echo yii::t('vcos', '品牌英文名')?></th>
                                                            <th><?php echo yii::t('vcos', '品牌国家')?></th>
                                                            <th><?php echo yii::t('vcos', '品牌logo')?></th>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['brand_id'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                                                            <td><?php echo $row['brand_cn_name'];?></td>
                                                            <td><?php echo $row['brand_en_name'];?></td>
                                                            <td><?php echo $row['country_cn_name'];?></td>
                                                            <td><img src="<?php echo Yii::app()->params['imgurl'].$row['brand_logo'];?>" width="50" height="50"></td>
                                                            <td><?php echo Helper::truncate_utf8_string($row['brand_desc'], 20);?></td>
                                                            <td><?php echo $row['sort_order'];?></td>
                                                            <td><?php echo $row['brand_status']?yii::t('vcos', '启用'):yii::t('vcos', '禁用');?></td>
                                                            <td>
                                                                <?php
                                                                    $this->widget('ManipulateWidget',array(
                                                                        'ControllerName'=>'Basic',
                                                                        'MethodName'=>'brand_edit',
                                                                        'id'=>$row['brand_id'],
                                                                        'canedit'=>$canedit,
                                                                        'candelete'=>$candelete,
                                                                    ));
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                        	var res = CheckPass();
                            if(res == 1){
                            	location.href="<?php echo Yii::app()->createUrl("Basic/brand_list");?>"+"?id="+$a;
                            }else if(res == 0){
                                alert('输入密码有误!');
                            }
                            //location.href="<?php echo Yii::app()->createUrl("Basic/brand_list");?>"+"?id="+$a;
