<?php
    $this->pageTitle = Yii::t('vcos','国家列表');
...
<?php 
    ...
    if(in_array('361', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('362', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('363', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th><?php echo yii::t('vcos', '国家名')?></th>
                                                            <th><?php echo yii::t('vcos', '国家英文名')?></th>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['country_id'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                                                            <td><?php echo $row['country_cn_name'];?></td>
                                                            <td><?php echo $row['country_en_name'];?></td>
                                                            <td><img src="<?php echo Yii::app()->params['imgurl'].$row['country_logo'];?>" width="50" height="50"></td>
                                                            <td><?php echo $row['status']?yii::t('vcos', '启用'):yii::t('vcos', '禁用');?></td>
                                                            <td>
                                                                <?php
                                                                    $this->widget('ManipulateWidget',array(
                                                                        'ControllerName'=>'Basic',
                                                                        'MethodName'=>'product_country_edit',
                                                                        'id'=>$row['country_id'],
                                                                        'canedit'=>$canedit,
                                                                        'candelete'=>$candelete,
                                                                    ));
                                                ...
                                                <div class="center">
                                                    <?php
                                                        ...
                                                        $this->widget('BotWidget',array(
                                                            'ControllerName'=>'Basic',
                                                            'MethodName'=>'product_country_add',
                                                            'canadd'=>$canadd,
                                                            'candelete'=>$candelete,
                                                        ));
                                                    ?>
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                        	var res = CheckPass();
                            if(res == 1){
                            	location.href="<?php echo Yii::app()->createUrl("Basic/product_country_list");?>"+"?id="+$a;
                            }else if(res == 0){
                                alert('输入密码有误!');
                            }
                            //location.href="<?php echo Yii::app()->createUrl("Basic/product_country_list");?>"+"?id="+$a;
