...
<?php 
    ...
    if(in_array('305', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('306', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('307', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th><?php echo yii::t('vcos', '导航名称')?></th>
                                                            <th><?php echo yii::t('vcos', '排序')?></th>
                                                            <th><?php echo yii::t('vcos', '导航类型')?></th>
                                                            <th><?php echo yii::t('vcos', '状态')?></th>
                                                            <th><?php echo yii::t('vcos', '是否显示')?></th>
                                                            <th><?php echo yii::t('vcos', '是否设置分类')?></th>
                                                            <th><?php echo yii::t('vcos', '是否第一个显示')?></th>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['navigation_id'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                                                            ...
                                                            <?php 
                                                            $type_arr = explode(',', $row['navigation_style_type']);
                                                            $data = '';
                                                            $data.= in_array('1',$type_arr)?"活动、 ":'';
                                                            $data.= in_array('2',$type_arr)?"店铺、 ":'';
                                                            $data.= in_array('3',$type_arr)?"商品、 ":'';
                                                            $data = trim($data,'、');?>
                                                            <td><?php echo $data;?></td>
                                                            <td><?php echo $row['status']?yii::t('vcos', '启用'):yii::t('vcos', '禁用');?></td>
                                                            <td><?php echo $row['is_show']?yii::t('vcos', '是'):yii::t('vcos', '否');?></td>
                                                            <td><?php echo $row['is_category']?yii::t('vcos', '是'):yii::t('vcos', '否');?></td>
                                                            <td><?php echo $row['is_main']?yii::t('vcos', '是'):yii::t('vcos', '否');?></td>
                                                            
                                                            <td>
                                                                <?php
                                                                    $this->widget('ManipulateWidget',array(
                                                                        'ControllerName'=>'Navigation',
                                                                        'MethodName'=>'navigation_edit',
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                        	var res = CheckPass();
                            if(res == 1){
                            	location.href="<?php echo Yii::app()->createUrl("Navigation/navigation_list");?>"+"?id="+$a;
                            }else if(res == 0){
                                alert('输入密码有误!');
                            }
                            //location.href="<?php echo Yii::app()->createUrl("Navigation/navigation_list");?>"+"?id="+$a;
