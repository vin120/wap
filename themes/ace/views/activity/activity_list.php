<?php
    $this->pageTitle = Yii::t('vcos','活动列表');
    $theme_url = Yii::app()->theme->baseUrl;
    $menu_type = 'activity_list';
?>
<?php 
    //navbar 挂件
    $this->widget('navbarWidget');
    if(in_array('300', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('301', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('302', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th><?php echo yii::t('vcos', '活动名称')?></th>
                                                            <th><?php echo yii::t('vcos', '活动描述')?></th>
                                                            <th><?php echo yii::t('vcos', '开始时间')?></th>
                                                            <th><?php echo yii::t('vcos', '结束时间')?></th>
                                                            <th><?php echo yii::t('vcos', '状态')?></th>
                                                            <th><?php echo yii::t('vcos', '是否显示活动头')?></th>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['activity_id'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                                                            <td><?php echo $row['activity_name'];?></td>
                                                            <td><?php echo Helper::truncate_utf8_string($row['activity_desc'], 20);?></td>
                                                            <td><?php echo $row['start_time'];?></td>
                                                            <td><?php echo $row['end_time'];?></td>
                                                            <td><?php echo $row['status']?yii::t('vcos', '启用'):yii::t('vcos', '禁用');?></td>
                                                            <td><?php echo $row['is_show_head']?yii::t('vcos', '显示'):yii::t('vcos', '不显示');?></td>
                                                            <td><img src="<?php echo Yii::app()->params['imgurl'].$row['activity_img'];?>" width="50" height="50"></td>
                                                            
                                                            <td>
                                                                <?php
                                                                    $this->widget('ManipulateWidget',array(
                                                                        'ControllerName'=>'Activity',
                                                                        'MethodName'=>'activity_edit',
                                                                        'id'=>$row['activity_id'],
                                                                        'canedit'=>$canedit,
                                                                        'candelete'=>$candelete,
                                                                    ));
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                        	var res = CheckPass();
                            if(res == 1){
                            	location.href="<?php echo Yii::app()->createUrl("Activity/activity_list");?>"+"?id="+$a;
                            }else if(res == 0){
                                alert('输入密码有误!');
                            }
                            //location.href="<?php echo Yii::app()->createUrl("Activity/activity_list");?>"+"?id="+$a;
