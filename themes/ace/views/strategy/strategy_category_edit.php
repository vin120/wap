<?php
    $this->pageTitle = Yii::t('vcos','编辑攻略分类');
    $this->pageTag = 'strategy_category_list';
    
    $theme_url = Yii::app()->theme->baseUrl;
    
    //$menu_type = 'strategy_category_add';
    $menu_type = 'strategy_category_list';
?>
<?php 
    //navbar 挂件
    $disable = 1;
                    ...
                    <div class="page-header">
                        <h1>
                            ...
                            <small style="cursor:pointer">
                                    ...
                                    <a href="<?php echo Yii::app()->createUrl("Strategy/strategy_category_list")?>"><?php echo yii::t('vcos', '攻略分类列表')?></a>
                            </small>
                            <small>
                                    <i class="icon-double-angle-right"></i>
                                    <?php echo yii::t('vcos', '编辑攻略分类')?>
                            </small>
                        </h1>
                    </div><!-- /.page-header -->
                    <div class="row">
                        <div class="col-xs-12 col-sm-12 col-md-11">
                            ...
                            ?> 
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-8 col-sm-8 col-md-7">
                                        <input type="text" id="name" placeholder="<?php echo yii::t('vcos', '分类名')?>" class="col-xs-10 col-sm-8 col-md-8" name="name" maxlength="80" value="<?php echo $strategy_category_language['category_name'] ?>" />
                                        <?php echo $form->error($strategy_category_language,'category_name'); ?> 
                                    </div>
                                </div>
                                <div class="space-4"></div>
                                ...
                                <div class="form-group">
                                    ...
                                    <label style="margin-left: 10px;">
                                        <input id="id-button-borders" type="checkbox" <?php if($strategy_category['state']){echo "checked='checked'";}?> class="ace ace-switch ace-switch-5" name="state" value="1" />
                                        <span class="lbl"></span>
                                    </label>
                                </div>
...
jQuery(function($){
    ...
    $(".iso_choose").click(function(){
        ...
        if($check == 'no'){
	        ...
	        $a = $(this).val();
	        $b = '<?php echo $_GET['id']?>';
	        $.post("<?php echo Yii::app()->createUrl("Strategy/getiso_category")?>",{iso:$a,id:$b},function(data){
	            if(data != 0){
	                var result = jQuery.parseJSON(data);
	                $(".iso input:text").val('');
	                $(".iso_name input:text").val(result['category_name']);
