                                ...
                                <tbody>
                                	...
                                	<!-- <php $child=0; if($key+1 < count($data)){if($data[$key+1]['parent_code'] == $row['goods_category_code']){$child = 1;}}?> -->
                                       <tr parent="<?php echo $row['parent_cid']?>" class="<?php if($row['level']!=1){echo "hidden";}?>" layer="<?php echo $row['level']?>" edit="0" val="<?php echo $row['cid']?>">
                                          ...
                                          <td><i hid="0" class="show_click <?php if($row['level']!=3){echo "hidden_child";}else{echo "blank_left";}?>"></i><label class="label_line">|<?php if($row['level']==1){echo '一';}elseif($row['level']==2){echo '一一一';}elseif($row['level']==3){echo '一一一一一';}?></label><input type="text" class="category_name hidden" value="<?php echo $row['name']?>" name="category_name[]" maxlength="40" /><span class="cat_name"><?php echo $row['name']?></span></td>
                                          <td><input style="width:50px;" type="text" value="<?php echo $row['sort_order']?>" class="category_sort hidden" maxlength="10" name="category_sort[]"/><span class="cat_sort"><?php echo $row['sort_order']?></span></td>
...
jQuery(function($){
	...
	$("#add_one_category").click(function(){
        ...
        str += '<td><i hid="0" class="show_click hidden_child"></i><label class="label_line">|一</label><input type="text" class="category_name" maxlength="40" value="" name="category_name[]"/><span class="cat_name hidden">aaa</span></td>';      
        str += '<td><input style="width:50px;" type="text" value="" class="category_sort" name="category_sort[]" maxlength="10"/><span class="cat_sort hidden">12</span></td>';    
	...
	$(document).on('click','.add_two_category',function(e){
	  ...
	  //查询是否存在其他地方正在操作
        ...
        str += '<td><i hid="0" class="show_click hidden_child"></i><label class="label_line">|一一一</label><input class="category_name" type="text" maxlength="40" value="" name="category_name[]"/><span class="cat_name hidden">bbb</span></td>';      
        str += '<td><input style="width:50px;" class="category_sort" maxlength="10" type="text" value="" name="category_sort[]"/><span class="cat_sort hidden">13</span></td>';    
    	str += '<td><label class="category_state" style="margin-left: 10px;"><input id="id-button-borders" type="checkbox" checked="checked" class="ace ace-switch ace-switch-5" name="state[]" value="1" /><span class="lbl"></span></label><span class="cat_state hidden">禁用</span></td>';
	...
	$(document).on('click','.add_three_category',function(e){
	  //查询是否存在其他地方正在操作
        ...
        str += '<td><i hid="0" class="show_click blank_left"></i><label class="label_line">|一一一一一</label><input type="text" class="category_name" value="" maxlegnth="40" name="category_name[]"/><span class="cat_name hidden">ccc</span></td>';      
        str += '<td><input style="width:50px;" class="category_sort" type="text" value="" maxlength="10" name="category_sort[]"/><span class="cat_sort hidden">14</span></td>';    
	...
	$(document).on('click','.add_finish',function(e){
		...
		var flag = 0;
		<?php $path_url = Yii::app()->createUrl('Dutyfreegoods/CheckCategoryNameExits');?>
	    $.ajax({
	        url:"<?php echo $path_url;?>",
	        type:'get',
	        data:'id='+this_id+'&name='+name,
	        async:false,
	     	dataType:'json',
	    	success:function(data){
	    		if(data==0) flag=1;
	...
	$("#start_all").click(function(){
		$("#many_goods_category > tbody").find("tr[parent!='0']").removeClass("hidden");
	});

	/**全部收起**/
	$("#stop_all").click(function(){
		$("#many_goods_category > tbody").find("tr[parent!='0']").addClass("hidden");
	});
});
