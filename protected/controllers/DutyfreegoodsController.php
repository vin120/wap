...
class DutyfreegoodsController extends Controller
{
    ...
    public function actionGoods_order_list()
    {
        ...
        $count_sql = "SELECT count(*) count FROM vcos_member_order WHERE ".$where." ORDER BY order_type ASC,is_read ASC, order_create_time";
        ...
        $pager->pageSize=10;
        $pager->applyLimit($criteria); 
        $sql = "SELECT * FROM vcos_member_order WHERE ".$where." ORDER BY order_type ASC,is_read ASC, order_create_time DESC LIMIT {$criteria->offset}, 10";  
        $result = Yii::app()->m_db->createCommand($sql)->queryAll();
        foreach ($result as $key=>$row){
            $sql = "SELECT cn_name FROM vcos_member WHERE member_code = {$row['membership_code']}";
            $result[$key]['cn_name'] = Yii::app()->db->createCommand($sql)->queryRow();
        }
        $this->render('goods_order_list',array('type'=>$type,'pages'=>$pager,'result'=>$result));
    }
    ...
    {
        $state = VcosMemberOrder::model()->findByPk($_POST['id']);
        $state->order_id = $_POST['id'];
        $state->order_status = $_POST['state'];
        $state->is_read = '1';
        if($state->save()>0){
            echo '1';
        }  else {
    ...
    public function actionMany_goods_category_data(){
    	...
    	if($_POST){
    		if($_POST['id'] == 0){
    			...
    			$sql = "UPDATE vcos_shop_operation_category SET status='{$_POST['state']}' WHERE category_code like '".$_POST['code']."%'";
    			$myself_operation = $db->createCommand($sql)->execute();
    			...
    			$myself_id = $db->createCommand($sql)->execute();
    			$sql = "UPDATE vcos_category SET status='{$_POST['state']}'  WHERE category_code like '".$_POST['code']."%'";
    			$db->createCommand($sql)->execute();
    			$success = 1;
    		}
    ...
    /**多级商品分类：验证分类名称是否已经存在**/
    public function actionCheckCategoryNameExits(){
    	$id = isset($_GET['id'])?$_GET['id']:0;
    	$name = isset($_GET['name'])?$_GET['name']:'';
    	if($id==0){
    		$sql = "SELECT count(*) count FROM `vcos_category` WHERE name='$name'";
    		$count = Yii::app()->p_db->createCommand($sql)->queryRow();
    	}else{
    		$sql = "SELECT count(*) count FROM `vcos_category` WHERE name='$name' AND cid!='{$id}'";
    		$count = Yii::app()->p_db->createCommand($sql)->queryRow();
    ...
    public function actionDel_many_goods_category(){
    	...
    	$sql = "UPDATE `vcos_shop_operation_category` set status=0 WHERE category_code like '{$code}%'";
    	$result_operation = $db->createCommand($sql)->execute();
    	/*
