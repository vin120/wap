...
class SafeController extends Controller
{
   ...
   /**安全须知列表**/
   public function actionSafe_info_list(){
   	$this->setauth();//检查有无权限
   	$db= Yii::app()->m_db;
   	//单条删除
   	if(isset($_GET['id'])){
   		$result = VcosSafe::model()->count();
   		if($result<=1){
   			die(Helper::show_message(yii::t('vcos', '不能把所有记录删除！')));
   		}
   		$did=$_GET['id'];
   		...
   		$transaction=$db->beginTransaction();
   		try{
   			$count = VcosSafe::model()->deleteByPk($did);
   			$count2 = VcosSafeLanguage::model()->deleteAll("safe_id in('$did')");
   			$transaction->commit();
   			Helper::show_message(yii::t('vcos', '删除成功。'), Yii::app()->createUrl("Safe/safe_info_list"));
   		}catch(Exception $e){
   			$transaction->rollBack();
   			Helper::show_message(yii::t('vcos', '删除失败。'));
   		}
   	}
   	$count_sql = "SELECT count(*) count FROM vcos_safe a LEFT JOIN vcos_safe_language b ON a.safe_id = b.safe_id WHERE b.iso = '".Yii::app()->language."'";
   	$count = $db->createCommand($count_sql)->queryRow();
   	//分页
   	$criteria = new CDbCriteria();
   	$count = $count['count'];
   	...
   	$pager->pageSize=10;
   	$pager->applyLimit($criteria);
   	$sql = "SELECT * FROM vcos_safe a LEFT JOIN vcos_safe_language b ON a.safe_id = b.safe_id WHERE b.iso = '".Yii::app()->language."' LIMIT {$criteria->offset}, {$pager->pageSize}";
   	$safe = $db->createCommand($sql)->queryAll();
   	$this->render('safe_info_list',array('pages'=>$pager,'auth'=>$this->auth,'safe'=>$safe));
   }
   
   /**安全信息添加**/
   public function actionSafe_info_add(){
   	$this->setauth();//检查有无权限
   	$safe = new VcosSafe();
   	$safe_language = new VcosSafeLanguage();
   	if($_POST){
   		$state = isset($_POST['state'])?$_POST['state']:'0';
   		...
   		try{
   			$safe->save();
   			if(isset($_POST['language']) && $_POST['language'] != ''){//判读是否同时添加系统语言和外语
   				$sql = "INSERT INTO `vcos_safe_language` (`safe_id`, `iso`, `safe_title`, `safe_content`) VALUES ('{$safe->primaryKey}', '".Yii::app()->params['language']."', '{$_POST['title']}', '{$_POST['describe']}'), ('{$safe->primaryKey}', '{$_POST['language']}', '{$_POST['title_iso']}', '{$_POST['describe_iso']}')";
   				$db->createCommand($sql)->execute();
   				$transaction->commit();
   				Helper::show_message(yii::t('vcos', '添加成功。'), Yii::app()->createUrl("Safe/safe_info_list"));
   				
   			}  else {//只添加系统语言时
   				$safe_language->safe_id = $safe->primaryKey;
   				$safe_language->iso = Yii::app()->params['language'];
   	...
   	$this->render('safe_info_add',array('safe'=>$safe,'safe_language'=>$safe_language));
   }
