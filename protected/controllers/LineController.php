...
class LineController extends Controller
{
        ...
        if($_POST){
            ...
            try{
                ...
                if(isset($_POST['language']) && $_POST['language'] != ''){//判读是否同时添加系统语言和外语
                    ...
                    Helper::show_message_query(yii::t('vcos', '添加成功,是否继续添加航线介绍？'),Yii::app()->createUrl("Line/line_detail_add"),Yii::app()->createUrl("Line/line_list"));
                }  else {//只添加系统语言时
    ...
    {   
        ...
        $count_sql = "SELECT count(*) count FROM vcos_line_detail a, vcos_line_detail_language b, vcos_line c, vcos_line_language d WHERE a.detail_id = b.detail_id AND a.line_id = c.line_id AND a.line_id = d.line_id AND b.iso = '".Yii::app()->language."' AND d.iso = '".Yii::app()->language."'  AND a.line_id='".$id."' ORDER BY a.detail_id DESC";
        $count = $db->createCommand($count_sql)->queryRow();
        $criteria = new CDbCriteria();
        $count = $count['count'];
        $pager = new CPagination($count);
        $pager->pageSize=10;
        $pager->applyLimit($criteria);
        $sql = "SELECT * FROM vcos_line_detail a, vcos_line_detail_language b, vcos_line c, vcos_line_language d WHERE a.detail_id = b.detail_id AND a.line_id = c.line_id AND a.line_id = d.line_id AND b.iso = '".Yii::app()->language."' AND d.iso = '".Yii::app()->language."' AND a.line_id='".$id."' ORDER BY a.detail_id DESC LIMIT {$criteria->offset}, {$pager->pageSize}";
        $detail = $db->createCommand($sql)->queryAll();
        $this->render('line_edit',array('current_page'=>$current_page,'line'=>$line,'line_language'=>$line_language,'pages'=>$pager,'auth'=>$this->auth,'detail'=>$detail));
    }
