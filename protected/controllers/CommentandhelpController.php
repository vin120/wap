...
class CommentandhelpController extends Controller
{
    ...
    public function actionSurvey_statistics()
    {
        ...
        $transaction=$db->beginTransaction();
        try{
        	$sql = "SELECT count(*) count FROM `vcos_survey_recode` WHERE survey_id={$id} AND (star_value=4 OR star_value=5)";
        	$data = $db->createCommand($sql)->queryRow();
        	$result[] = $data['count'];
        	$sql = "SELECT count(*) count FROM `vcos_survey_recode` WHERE survey_id={$id} AND (star_value=2 OR star_value=3)";
        	$data = $db->createCommand($sql)->queryRow();
        	$result[] = $data['count'];
        	$sql = "SELECT count(*) count FROM `vcos_survey_recode` WHERE survey_id={$id} AND star_value<2";
        	$data = $db->createCommand($sql)->queryRow();
        	$result[] = $data['count'];
        	$transaction->commit();
        }catch(Exception $e){
        ...
        $this->render('survey_statistics',array('result'=>$result,'title'=>$title));
    }
    ...
    public function actionUser_comment_detail()
    {
        $this->setauth();//检查有无权限
        $id = $_GET['id'];
        $sql = "SELECT c.comment_type_name, a.comment_time, a.comment_content, a.membership_code
                FROM vcos_comment a, vcos_comment_type_language c
        ...
        $sql = "SELECT * FROM vcos_member WHERE member_id = {$comment['membership_code']}";
    ...
    /**评价结果**/
    public function actionSurvey_recode(){
    	$this->setauth();//检查有无权限
        $db = Yii::app()->m_db;
        $survey_but = isset($_GET['survey'])?$_GET['survey']:0;
        if($survey_but==0){
        	$where = 1;
        }else{
        	$where = "a.survey_id='{$survey_but}'";
        }
        $count_sql = "SELECT count(*) count FROM vcos_survey_recode a LEFT JOIN vcos_survey_language b ON a.survey_id = b.survey_id WHERE b.iso = '".Yii::app()->language."' AND ".$where." ORDER BY a.survey_id DESC";
        $count = Yii::app()->m_db->createCommand($count_sql)->queryRow();
        $criteria = new CDbCriteria();
        $count = $count['count'];
        $pager = new CPagination($count);
        $pager->pageSize=10;
        $pager->applyLimit($criteria);
        $sql = "SELECT a.*,b.survey_title FROM vcos_survey_recode a LEFT JOIN vcos_survey_language b ON a.survey_id = b.survey_id WHERE b.iso = '".Yii::app()->language."' AND ".$where." ORDER BY a.survey_id DESC LIMIT {$criteria->offset}, {$pager->pageSize}";
        $recode = Yii::app()->m_db->createCommand($sql)->queryAll();
        $sql = "SELECT a.survey_id,b.survey_title FROM `vcos_survey` a LEFT JOIN `vcos_survey_language` b ON a.survey_id=b.survey_id WHERE b.iso='".Yii::app()->language."' AND a.survey_state=1";
        $survey = Yii::app()->m_db->createCommand($sql)->queryAll();
        $this->render('survey_recode',array('survey_but'=>$survey_but,'survey'=>$survey,'pages'=>$pager,'auth'=>$this->auth,'recode'=>$recode));
