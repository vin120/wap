...
class AdController extends Controller {
	...
	public function actionAd_add() {
		...
		$this->render ( 'ad_add', array (
				'title_en_sel' => $title_en_sel,
				'title_sel' => $title_sel,
				'position' => $position,
				'ad' => $ad,
		...
		if ($_POST) {
			...
			try {
				if (isset ( $_POST ['language'] ) && $_POST ['language'] != '') { // 编辑系统语言和外语状态下
					...
					$db->createCommand ()->update ( 'vcos_ad', $columns, 'ad_id = :id', array (
							':id' => $id 
					) );
					// 编辑系统语言
					$l_colums = array (
					...
					$db->createCommand ()->update ( 'vcos_ad_language', $l_colums, 'id=:id', array (
							':id' => $id2 ['id'] 
					) );
					// 判断外语是新增OR编辑
					if ($_POST ['judge'] == 'add') {
						...
						$columns = array (
								'ad_id' => $id,
								'iso' => $_POST ['language'],
								'name' => $_POST ['title_iso'],
								'link_url' => $link_url_iso 
						);
						if ($photo_iso) { // 判断有无上传图片
						...
						$db->createCommand ()->update ( 'vcos_ad_language', $columns, 'id=:id', array (
								':id' => $_POST ['judge'] 
						) );
					}
					// 事务提交
