...
class LifeserviceController extends Controller
{
    ...
    public function actionService_add()
    {
        ...
        if($_POST){
            ...
            $ls->old_price = $_POST['old_price']*100;
            $ls->ls_tel = $_POST['tel'];
        ...
        if($_POST){
            ...
            try{
                if(isset($_POST['language']) && $_POST['language'] != ''){//编辑系统语言和外语状态下
                    //编辑主表
                    $columns = array('old_price'=>($_POST['old_price']*100),'ls_price'=>($_POST['price']*100),'ls_state'=>$state,'ls_category'=>$_POST['type'],'ls_tel'=>$_POST['tel']);
                    if($photo){//判断有无上传图片
    ...
    public function actionService_booking_list()
    {
        ...
        $count_sql = "SELECT count(*) count FROM vcos_member_booking WHERE ".$where." ORDER BY create_time DESC ";
        ...
        $pager->pageSize=10;
        $pager->applyLimit($criteria); 
        $sql = "SELECT * FROM vcos_member_booking WHERE ".$where." ORDER BY create_time DESC LIMIT {$criteria->offset}, 10";
        $result = Yii::app()->m_db->createCommand($sql)->queryAll();
        foreach ($result as $key=>$row){
            $sql = "SELECT cn_name FROM vcos_member WHERE member_code = {$row['member_code']} LIMIT 1";
            $result[$key]['cn_name'] = Yii::app()->db->createCommand($sql)->queryRow();
            if($row['booking_type']==1){
                $sql = "SELECT restaurant_name as store_name FROM vcos_restaurant_language WHERE restaurant_id='{$row['store_id']}' AND iso='".Yii::app()->language."' LIMIT 1";
                $result[$key]['store_name'] = Yii::app()->db->createCommand($sql)->queryRow();
            }else if($row['booking_type']==2){
                $sql = "SELECT ls_title as store_name FROM vcos_lifeservice_language WHERE ls_id='{$row['store_id']}' AND iso='".Yii::app()->language."' LIMIT 1";
                $result[$key]['store_name'] = Yii::app()->db->createCommand($sql)->queryRow();
            }
        }
        $this->render('service_booking_list',array('type'=>$type,'pages'=>$pager,'result'=>$result));
    }
    ...
    public function actionService_booking_edit()
    {
        ...
        $sql = "SELECT cn_name FROM vcos_member WHERE member_code = {$detail->member_code}";
        $user = Yii::app()->db->createCommand($sql)->queryRow();
        if($detail->booking_type==1){
            $sql = "SELECT restaurant_name as store_name FROM vcos_restaurant_language WHERE restaurant_id='{$detail->store_id}' AND iso='".Yii::app()->language."' LIMIT 1";
        }else{
            $sql = "SELECT ls_title as store_name FROM vcos_lifeservice_language WHERE ls_id='{$detail->store_id}' AND iso='".Yii::app()->language."' LIMIT 1";
        }
        $store = Yii::app()->db->createCommand($sql)->queryRow();
        $this->render('service_booking_edit',array('detail'=>$detail,'user'=>$user,'store'=>$store));
    }
    
    public function actionChangestate()
    {
        $state = VcosMemberBooking::model()->findByPk($_POST['id']);
        $state->id = $_POST['id'];
        $state->status = $_POST['state'];
        $state->is_read = 1;
        if($state->save()>0){
            echo '1';
        }  else {
