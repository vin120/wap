...
class RestaurantController extends Controller
{
    ...
    public function actionRestaurant_add()
    {
        ...
        if($_POST){
            ...
            $shipping_fee = isset($_POST['shipping_fee'])?trim($_POST['shipping_fee'])*100:'0';
            $box_price = isset($_POST['box_price'])?trim($_POST['box_price'])*100:'0';
            $restaurant->restaurant_tel = $_POST['tel'];
            $restaurant->restaurant_state = $state;
            $restaurant->can_delivery = $delivery;
            if($delivery==1){
                $restaurant->shipping_fee = $shipping_fee;
                $restaurant->box_price = $box_price;
            }
        ...
        if($_POST){
            ...
            try{
                if(isset($_POST['language']) && $_POST['language'] != ''){//编辑系统语言和外语状态下
                    //编辑主表
                    $columns = array('shipping_fee'=>$shipping_fee,'box_price'=>$box_price,'restaurant_type'=>$_POST['type'],'restaurant_tel'=>$_POST['tel'],'restaurant_state'=>$state,'restaurant_sequence'=>$_POST['sequence'],'bg_color'=>$_POST['bgcolor'],'can_delivery'=>$delivery,'can_book'=>$book);
                    if($photo1){//判断有无上传图片
    ...
    public function actionFood_add()
    {
        ...
        if($_POST){
            ...
            $food->old_price = $_POST['old_price']*100;
     ...
     public function actionFood_edit()
    {
        ...
        if($_POST){
            ...
            try{
                if(isset($_POST['language']) && $_POST['language'] != ''){//编辑系统语言和外语状态下
                    //编辑主表
                    $columns = array('old_price'=>($_POST['old_price']*100),'food_price'=>($_POST['price']*100),'food_state'=>$state,'max_buy'=>$_POST['max_buy'],'food_category_id'=>$_POST['food_category'],'restaurant_id'=>$_POST['restaurant']);
                    if($photo){//判断有无上传图片
    ...
    public function actionCheckCategoryName(){
        ...
        $restaurant = isset($_GET['restaurant'])?trim($_GET['restaurant']):0;
        $name = isset($_GET['name'])?trim($_GET['name']):'';
        $name_iso = isset($_GET['name_iso'])?trim($_GET['name_iso']):'';
        if($id==0){
            if($name_iso!=''){
            $sql = "SELECT count(*) count FROM `vcos_food_category` a LEFT JOIN `vcos_food_category_language` b ON a.food_category_id=b.food_category_id WHERE (b.food_category_name='{$name}' AND b.iso='zh_cn' ) OR (b.food_category_name='{$name_iso}' AND b.iso='en') AND a.restaurant_id='{$restaurant}'";
            }else{
                $sql = "SELECT count(*) count FROM `vcos_food_category` a LEFT JOIN `vcos_food_category_language` b ON a.food_category_id=b.food_category_id WHERE b.food_category_name='{$name}' AND b.iso='zh_cn' AND  a.restaurant_id='{$restaurant}'";
            }
        }else{
            if($name_iso!=''){
            $sql = "SELECT count(*) count FROM `vcos_food_category` a LEFT JOIN `vcos_food_category_language` b ON a.food_category_id=b.food_category_id WHERE ((b.food_category_name='{$name}' AND b.iso='zh_cn' ) OR (b.food_category_name='{$name_iso}' AND b.iso='en')) AND a.food_category_id!='{$id}' AND a.restaurant_id='{$restaurant}'";
            }else{
                $sql = "SELECT count(*) count FROM `vcos_food_category` a LEFT JOIN `vcos_food_category_language` b ON a.food_category_id=b.food_category_id WHERE b.food_category_name='{$name}' AND b.iso='zh_cn'  AND a.food_category_id!='{$id}' AND a.restaurant_id='{$restaurant}'";
            }
        }
