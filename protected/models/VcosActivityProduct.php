...
class VcosActivityProduct extends PrCActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('product_id, activity_cid, sort_order, product_type, is_overdue', 'numerical', 'integerOnly'=>true),
			array('activity_id', 'length', 'max'=>10),
			array('start_show_time, end_show_time', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, activity_id, product_id, activity_cid, sort_order, start_show_time, end_show_time, product_type, is_overdue', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('is_overdue',$this->is_overdue);
